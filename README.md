...
**Option G: AmpleGCG Baseline (Shared 160-Suffix Pools)**

AmpleGCG 分两步：先基于四个模型 attack pool 的并集，为每个 task 生成一份共享的 `160` suffix 池；再在各 target model 上复用同一份 suffix 池进行攻击。suffix 文件保存在 `results/amplegcg_160/`，攻击结果保存在 `results/amplegcg/`。

```bash
cd scripts/amplegcg
mkdir -p ../../logs/amplegcg_160 ../../results/amplegcg ../../results/amplegcg_160

# 1) 先生成全部 task 的共享 160-suffix pools
nohup bash run_amplegcg_generate_160_all.sh > ../../logs/amplegcg_generate_160_all.log 2>&1 &

# 2) 再按目标模型分别攻击
nohup bash run_amplegcg_attack_finma_160.sh    > ../../logs/amplegcg_attack_finma_160.log 2>&1 &
nohup bash run_amplegcg_attack_xuanyuan_160.sh > ../../logs/amplegcg_attack_xuanyuan_160.log 2>&1 &
nohup bash run_amplegcg_attack_fingpt_160.sh   > ../../logs/amplegcg_attack_fingpt_160.log 2>&1 &
nohup bash run_amplegcg_attack_finr1_160.sh    > ../../logs/amplegcg_attack_finr1_160.log 2>&1 &
```
...
**Option H: Random Baseline 160 补充任务**

针对 Random Baseline（budget=160, suffix=30 tokens）的 4 组补充实验，每组只需**单卡**，可分卡并行：

| # | Target Model | Task | n_samples | 脚本 |
|---|-------------|------|-----------|------|
| 1 | Fin-R1 | flare_fiqasa | 96 | `run_random_160_finr1_fiqasa.sh` |
| 2 | Fin-R1 | fintrust_fairness | 300 | `run_random_160_finr1_fintrust.sh` |
| 3 | XuanYuan-6B | flare_headlines | 300 | `run_random_160_xuanyuan_headlines.sh` |
| 4 | XuanYuan-6B | fintrust_fairness | 129 | `run_random_160_xuanyuan_fintrust.sh` |

```bash
cd scripts/random
mkdir -p ../../logs/random_baseline_160 ../../result/random_baseline_160

# 同模型的两组可在同一张卡上串行，不同模型可分卡并行
nohup bash run_random_160_finr1_fiqasa.sh      > ../../logs/random_160_finr1_fiqasa.log 2>&1 &
nohup bash run_random_160_finr1_fintrust.sh    > ../../logs/random_160_finr1_fintrust.log 2>&1 &
nohup bash run_random_160_xuanyuan_headlines.sh > ../../logs/random_160_xuanyuan_headlines.log 2>&1 &
nohup bash run_random_160_xuanyuan_fintrust.sh  > ../../logs/random_160_xuanyuan_fintrust.log 2>&1 &
```
